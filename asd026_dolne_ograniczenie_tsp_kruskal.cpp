...
void Kruskal(Graph* gr, int start=0){
    ...
    ///wlasciwe pobieranie z kolejki
    while(!IsEmpty(Q)){
        ///pobierz element i zwin jesli trzeba sciezki w union findzie
        x = GetMin(Q);
        for(i=ds[x.a]; i!=ds[i]; i=ds[i]); ds[x.a] = i;
        for(i=ds[x.b]; i!=ds[i]; i=ds[i]); ds[x.b] = i;

        ///jesli ten sam set
        ///nie rob nic ciekawego, bo to nie ma znaczenia
        ///nie mozesz dodac krawedzi bo powstalby cykl
        if(ds[x.a] == ds[x.b]){
            if(x.cost < najkrotszy_z_pozostalych){
                najkrotszy_z_pozostalych = x.cost;
            }
        }
        ///jak rozne sety to union
        else{
            ds[ds[x.b]] = ds[x.a];
